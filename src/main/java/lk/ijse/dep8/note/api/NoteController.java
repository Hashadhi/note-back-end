...
public class NoteController {
    ...
    public NoteDTO addNote(@PathVariable String userId, @RequestBody NoteDTO note, Errors errors) {
        if (errors.hasFieldErrors()) {
            throw new ResponseStatusException(HttpStatus.BAD_REQUEST, errors.getFieldErrors().get(0).getDefaultMessage());
        }

        if(!userId.equals(note.getUserId())) throw new ResponseStatusException(HttpStatus.CONFLICT, "User id mismatch");
